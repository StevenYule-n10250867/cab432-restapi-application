...
router.get('/:id/report', allowPublicReports ? passThrough : authMiddleware, (req, res) => {
  ...
  const thumbs = Array.isArray(job.thumbnails) ? job.thumbnails : [];
...
<style>
  ...
  .thumbs img{height:100px;margin-right:6px;border:1px solid #ddd;border-radius:6px}
...
<p>Status: <b>${esc(job.status)}</b>
 &middot; Started: ${esc(job.startedAt || '')}
 &middot; Finished: ${esc(job.finishedAt || '')}
 &middot; Elapsed: ${esc(has(job.elapsedMs) ? job.elapsedMs : '')} ms</p>
...
${thumbs.length ? `<div class="section"><h2>Thumbnails</h2><div class="thumbs">${
  thumbs.map(t => `<img src="${esc(t)}" alt="thumb">`).join('')
}</div></div>` : ''}
